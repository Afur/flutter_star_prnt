...
public class FlutterStarPrntPlugin : FlutterPlugin, MethodCallHandler {
    ...
    // EDIT: this should be async right now, but might not work since it's implementation without test on device
    ...
    public fun disconnect(@NonNull call: MethodCall, @NonNull result: Result) {
        val starIoExtManager = this.starIoExtManager
        if (starIoExtManager != null) {
            starIoExtManager.disconnect(object : IConnectionCallback {
                override fun onConnected(connectResult: IConnectionCallback.ConnectResult) {
                }
                ...
                override fun onDisconnected() {
                    result.success("Printer Disconnected")
                }
            })
        } else {
            result.success("Printer Disconnected")
    ...
    private fun appendCommands(
        ...
        printCommands?.forEach {
            if (it.containsKey("appendCharacterSpace"))
                ...
                builder.appendInternational(
                    getInternational(
                        it.get("appendInternational").toString()
                    )
                )
